...
from app.search.client import SearchApiClient
from app.shared.config import configure_logging, load_config
from app.shared.media import create_input_file
...
async def perform_search(chat_id: int, query: str, user_id: int, search_session_token: str = "") -> None:
    try:
        ...
        if search_session_token:
            result = await asyncio.to_thread(search_api.next_search_results, search_session_token, user_id, 5)
        else:
